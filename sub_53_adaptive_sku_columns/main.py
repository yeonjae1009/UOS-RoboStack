...
def main() -> None:
    ...
    algo_cfg = AlgorithmConfig(
        ...
        min_support=float(cfg["algorithm"].get("min_support", 0.70)),
        emergency_support=float(cfg["algorithm"].get("emergency_support", 0.45)),
        beam_width=int(cfg["algorithm"].get("beam_width", 8)),
        beam_depth=int(cfg["algorithm"].get("beam_depth", 1)),
        positions_per_box=int(cfg["algorithm"].get("positions_per_box", 4)),
        enable_adaptive_sku_columns=bool(
            cfg["algorithm"].get("enable_adaptive_sku_columns", True)
        ),
        sku_column_duplicate_count=int(
            cfg["algorithm"].get("sku_column_duplicate_count", 2)
        ),
        sku_column_max_height=float(
            cfg["algorithm"].get("sku_column_max_height", 0.85)
        ),
        sku_column_min_footprint_area=float(
            cfg["algorithm"].get("sku_column_min_footprint_area", 0.0)
        ),
        sku_column_max_footprint_area=float(
            cfg["algorithm"].get("sku_column_max_footprint_area", 0.060)
        ),
        sku_column_confirmation_boxes=int(
            cfg["algorithm"].get("sku_column_confirmation_boxes", 15)
        ),
        sku_column_min_observed_types=int(
            cfg["algorithm"].get("sku_column_min_observed_types", 5)
        ),
        sku_column_large_aspect_ratio_min=float(
            cfg["algorithm"].get("sku_column_large_aspect_ratio_min", 1.10)
        ),
    )
