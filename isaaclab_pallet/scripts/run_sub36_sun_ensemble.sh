#!/usr/bin/env bash
# Fine-tune with the sub36 strategy-ensemble candidate generator and terminal-explore reward profile.
#
# Launch:
#   nohup bash isaaclab_pallet/scripts/run_sub36_sun_ensemble.sh > /tmp/sub36_sun_ensemble.out 2>&1 &
...
OUT_DIR="${OUT_DIR:-isaaclab_pallet/runs/sub36_sun_ensemble_random50}"
RUN_NAME="${RUN_NAME:-sun_v2_terminal_explore_sub36_sun_ensemble}"
PCT_CONFIG_PATH="${PCT_CONFIG_PATH:-templete code/config/pct_config_sub36_sun_ensemble.yaml}"
...
LR="${LR:-5e-7}"
BOX_SEED="${BOX_SEED:-0}"
CANDIDATE_RERANK_K="${CANDIDATE_RERANK_K:-4}"
...
WANDB_GROUP="${WANDB_GROUP:-sub36_sun_ensemble_random50}"
WANDB_MODE="${WANDB_MODE:-}"
WANDB_DIR="${WANDB_DIR:-}"
WANDB_TAGS="${WANDB_TAGS:-sub36_sun_ensemble random50 sun_v2_terminal_explore k4}"
...
if [ ! -s "$WARM_START" ]; then
  echo "[sub36-sun-ensemble] missing WARM_START=$WARM_START" | tee -a "$log"
  exit 1
fi
if [ ! -s "$PCT_CONFIG_PATH" ]; then
  echo "[sub36-sun-ensemble] missing PCT_CONFIG_PATH=$PCT_CONFIG_PATH" | tee -a "$log"
  exit 1
fi
...
echo "[sub36-sun-ensemble] start $(date)" | tee -a "$log"
echo "[sub36-sun-ensemble] out_dir=$OUT_DIR run_name=$RUN_NAME pct_config=$PCT_CONFIG_PATH warm_start=$WARM_START" | tee -a "$log"
echo "[sub36-sun-ensemble] num_envs=$NUM_ENVS packer_workers=$NUM_PACKER_WORKERS max_boxes=$MAX_BOXES updates=$UPDATES lr=$LR candidate_k=$CANDIDATE_RERANK_K" | tee -a "$log"
echo "[sub36-sun-ensemble] eval_box_seq_dir=$EVAL_BOX_SEQ_DIR eval_count=${#eval_sequences_array[@]} eval_sequences=${eval_sequences_array[*]}" | tee -a "$log"
...
python3 isaaclab_pallet/scripts/train_pallet_gat.py \
  ...
  --output-dir "$OUT_DIR" \
  --run-name "$RUN_NAME" \
  --reward-profile sun_v2_terminal_explore \
  --num-envs "$NUM_ENVS" \
...
echo "[sub36-sun-ensemble] exited code=$code $(date)" | tee -a "$log"
exit "$code"
