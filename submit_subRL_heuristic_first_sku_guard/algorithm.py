...
try:
    import heuristic_policy as sku_heuristic
except Exception:
    sku_heuristic = None
...
class Palletizer:
    ...
    def _reset_state(self) -> None:
        ...
        self._observed_sku_keys: set[Tuple[int, int]] = set()
        self._observed_sku_counts: Dict[Tuple[float, float, float], int] = {}
        self._first_footprint_area: Optional[float] = None
        self._rl_mode_active = False
        self._rl_rebuild_done = False
        self._sku_heuristic = None
        self._pending_heuristic_placed = False
    ...
    def should_finish(self, current_buffer: List[BoxInput]) -> bool:
        ...
        for box in current_buffer:
            self._observe_sku_box(box)
        if self._uses_heuristic_mode():
            self._maybe_switch_to_rl()
            return False
    ...
    def _observe_sku_box(self, box: BoxInput) -> None:
        box_key = self._box_key(box)
        if box_key in self._observed_sku_keys:
            return
        self._observed_sku_keys.add(box_key)
        signature = self._sku_signature(box)
        if self._first_footprint_area is None:
            self._first_footprint_area = signature[0] * signature[1]
        self._observed_sku_counts[signature] = (
            self._observed_sku_counts.get(signature, 0) + 1
        )
        if len(self._observed_sku_counts) > 8:
            return

    def _uses_heuristic_mode(self) -> bool:
        return not self._rl_mode_active

    def _is_sub56_like_sku_observation(self) -> bool:
        observed_count = len(self._observed_sku_keys)
        if observed_count < 8:
            return False

        unique_count = len(self._observed_sku_counts)
        if unique_count > 7:
            return False

        counts = list(self._observed_sku_counts.values())
        max_duplicate = max(counts, default=0)
        if max_duplicate < 2:
            return False

        repeated_types = sum(1 for count in counts if count >= 2)
        largest = max(
            self._observed_sku_counts,
            key=lambda signature: signature[0] * signature[1],
        )
        largest_aspect = largest[1] / max(largest[0], 1e-9)

        medium_start = (
            self._first_footprint_area is not None
            and 0.055 <= self._first_footprint_area <= 0.065
        )
        large_repeated = observed_count >= 15 and largest_aspect >= 1.10
        dense_repeated = repeated_types >= 3 and max_duplicate >= 3
        return medium_start or large_repeated or dense_repeated

    def _is_non_sku_observation(self) -> bool:
        observed_count = len(self._observed_sku_keys)
        if observed_count < 15:
            return False
        if self._is_sub56_like_sku_observation():
            return False

        unique_count = len(self._observed_sku_counts)
        repeated_types = sum(
            1 for count in self._observed_sku_counts.values() if count >= 2
        )
        return unique_count >= 12 and repeated_types <= 1

    def _maybe_switch_to_rl(self) -> None:
        if self._rl_mode_active:
            return
        if self._is_non_sku_observation():
            self._rl_mode_active = True
            self._rebuild_rl_packer_from_sequence()

    def _find_position_sku_heuristic(
        self,
        box: BoxInput,
    ) -> Optional[Tuple[float, float, float, Tuple[float, float, float], int]]:
        if sku_heuristic is None:
            return None
        if self._sku_heuristic is None:
            self._sku_heuristic = sku_heuristic.Palletizer(
                sku_heuristic.PalletConfig(
                    float(self.pallet.length),
                    float(self.pallet.width),
                    float(self.pallet.height),
                ),
                sku_heuristic.AlgorithmConfig(
                    allow_rotation=bool(self.algo.allow_rotation),
                    buffer_size=0,
                ),
            )
        self._sku_heuristic.sequence = copy.deepcopy(self.sequence)
        self._sku_heuristic.solids = []
        found = self._sku_heuristic._find_position(box)
        if found is not None:
            self._pending_heuristic_placed = True
        return found

    def _rebuild_rl_packer_from_sequence(self) -> bool:
        if self._rl_rebuild_done:
            return True
        ...
        for placed in self.sequence:
            size = [float(v) for v in placed["size"]]
            position = [float(v) for v in placed["position"]]
            lx = round(position[0] - size[0] / 2.0, 6)
            ly = round(position[1] - size[1] / 2.0, 6)
            density = self._density(
                {
                    "step": int(placed["step"]),
                    "id": int(placed["id"]),
                    "size": size,
                    "mass": float(placed["mass"]),
                },
                size,
            )
            self._packer.observe(size, density)
            leaf = np.asarray(
                [lx, ly, 0.0, lx + size[0], ly + size[1], self.pallet.height],
                dtype=np.float64,
            )
            if not self._packer.place(leaf):
                return False
            self._raw_sequence.append(placed)

        self._rl_rebuild_done = True
        return True
    ...
    def _find_position(
        ...
        if self._uses_heuristic_mode():
            return self._find_position_sku_heuristic(box)

        if not self._rebuild_rl_packer_from_sequence():
            return None
    ...
    def _append_placed(
        ...
        center_y = y + dy / 2.0
        center_z = z + dz / 2.0
        if not self._pending_heuristic_placed:
            margin = 0.002
            if center_x - dx / 2.0 < 0.0:
                center_x += margin
            if center_y - dy / 2.0 < 0.0:
                center_y += margin
        ...
        self.sequence.append({
            ...
            "mass": float(box["mass"]),
            "position": [
                round(center_x, precision),
                round(center_y, precision),
                round(center_z, precision),
            ],
            "rotation": int(rotation),
        })
        self._pending_heuristic_placed = False
