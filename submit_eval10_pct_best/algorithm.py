...
class Palletizer:
    ...
    def _ensure_policy(self) -> None:
        ...
        self._candidate_generator = str(cfg.get("candidate_generator", "ems"))
    ...
    def _validated_output_from_packed(
        ...
        if (
            ...
            or ly < -eps
            or lz < -eps
            or lx + dx > float(self.pallet.length) + eps
            or ly + dy > float(self.pallet.width) + eps
            or lz + dz > float(self.pallet.height) + eps
        ):
            return None
