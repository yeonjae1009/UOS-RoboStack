...
def get_args():
    ...
    # [CJ] 진짜 이어하기: 가중치만 불러오는 --load-model 과 달리, 옵티마이저(Adam) 상태 +
    #      step 카운터 + best 기록까지 복원해 "쭉 돌린 것"과 동일하게 학습을 이어간다.
    #      PCT-resume.pt(번들) 파일을 사용. (--load-model 과 함께 쓰지 말 것)
    parser.add_argument('--resume', action='store_true', help='[CJ] True-resume: restore weights + optimizer + step + best')
    parser.add_argument('--resume-path', type=str, default='', help='[CJ] Path to PCT-resume.pt bundle')
