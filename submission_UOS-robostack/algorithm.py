...
class Palletizer:
    ...
    def __init__(self, pallet_cfg: PalletConfig, algo_cfg: AlgorithmConfig) -> None:
        ...
        # setting 3 = 질량(밀도)을 EMS 안정성 판정에 반영. setting<3 이면 density=1.
    ...
    def run(self, boxes: List[BoxInput]) -> RunResult:
        ...
        for box in boxes:
            ...
            # 1) 현재 상태 관찰값 생성
            #    setting 3 학습 모델은 박스 밀도(질량/부피/DENSITY_MAX)를 입력으로 받으므로
            #    추론 때도 동일하게 넘겨야 train/deploy 가 일치한다(주의 2). setting<3 이면 density=1.
            if self.setting >= 3:
